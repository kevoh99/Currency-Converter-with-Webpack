...
function getToWork (event) {
  ...
  answerContainer.style.backgroundColor = 'var(--main-bg-color)'
  answerContainer.style.color = 'var(--primary-color)'
  answerContainer.style.fontSize = '1.2rem'
...
const atWork = async () => {
  ...
  try {
    ...
    answerContainer.innerText = soln
  } catch (error) {
...
const getExchangeRate = async (fromCurrency, toCurrency) => {
  try {
    ...
    const response = await axios.get('http://api.currencylayer.com/live?access_key=af908b96fc9d56f13d11929909a54b1c')
    ...
    throw new Error(`Unable to get currency data using ${fromCurrency} and ${toCurrency}. Please try again.\n If the problem persists, please contact us`)
  }
}

const getCountries = async (currencyCode) => {
  try {
    const response = await axios.get(`http://api.countrylayer.com/v2/currency/${currencyCode}?access_key=e2babe7843010bd817bf049122dcefa4`)
    const countries = response.data.map(country => country.name)
    return countries
  } catch (error) {
    throw new Error(`Sorry. Our system was unable to find countries that use ${currencyCode}.`)
  }
}
...
const convert = async (fromCurrency, toCurrency, amount) => {
  ...
  return `${amount} ${currencyChart[fromCurrency]} equals ${convertedAmount} ${currencyChart[toCurrency]}. You can use ${currencyChart[toCurrency]} in: ${countries}`
}
